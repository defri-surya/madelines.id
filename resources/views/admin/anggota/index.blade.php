            ...
            <div class="grid grid-cols-1 p-4 space-y-8 lg:gap-12 lg:space-y-0 lg:grid-cols-12">
                ...
                <div class="col-span-2 bg-white rounded-md dark:bg-darker" x-data="{ isOn: false }">
                    ...
                    <div class="flex items-center justify-between p-4 border-b dark:border-primary">
                        <h4 class="text-lg font-semibold text-gray-500 dark:text-light">Struktur Member</h4>
                    </div>
                    <!-- Chart -->
                    <div class="relative p-4">
                        <div class="body genealogy-body genealogy-scroll">
                            <div class="genealogy-tree">
                                @include('admin.anggota.treeAll', ['node' => $tree])
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="w-full table-auto">
                            <tbody>
                                @php
                                    $no = 1;
                                @endphp
                                @forelse ($treeCM as $child)
                                    <tr>
                                        <td>{{ $no++ }}.</td>
                                        <td>{{ $child->name }}</td>
                                        <td>{{ $child->referal }}</td>
                                        <td>By Referal : {{ $child->by_referal }}</td>
                                        <td>{{ $child->no_hp }}</td>
                                    </tr>
                                @empty
